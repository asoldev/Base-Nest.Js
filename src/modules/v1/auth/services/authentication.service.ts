...
export class AuthenticationService {
  ...
  public async signIn(email: string, pass: string): Promise<any> {
    const user = BaseDto.plainToClass(
      User,
      await this.dataService.users.findOneByFilter({ email })
    );
    if (!user) {
  ...
  public async register(
    registerDto: AuthenticationRequestDto.AuthenticationRegisterDto
  ) {
    const { email, password, first_name, last_name } = registerDto;
    const existingUser = await this.dataService.users.findOneByFilter({
      email,
    });
    ...
    return BaseDto.plainToClass(
      User,
      await this.dataService.users.insertOne(document)
    );
  }
}
